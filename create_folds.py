...
from iterstrat.ml_stratifiers import MultilabelStratifiedKFold # To create stratified folds
...
def create_fold(df, save_path="input/train_folds.csv"):
    ...
    '''
    df["kfold"] = -1 # Create a new column in the DataFrame called kfold to store the folds
    df = df.sample(frac=1).reset_index(drop=True) # Shuffle the data
    y = df.drop(["sig_id"], axis=1).values # Get the labels by dropping the sig_id column beacuse it is not a target

    mskf = MultilabelStratifiedKFold(n_splits=CFG.n_splits) # Create the Multi-Label Stratified K-Fold object by passing in the number of folds from the config file
    for fold, (train_idx, val_idx) in enumerate(mskf.split(df, y)): # Loop through the folds and get trn_idx and val_idx for each fold
        print(len(train_idx), len(val_idx))
        df.loc[val_idx, "kfold"] = fold # Store the fold number in the kfold column for the validation indices of that particular fold
